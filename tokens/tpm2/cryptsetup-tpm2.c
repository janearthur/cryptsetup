...
static int action_tpm2_add(struct crypt_device *cd)
{
	...
	r = tools_get_key("Enter existing LUKS2 pasphrase:",
			  &existing_pass, &existing_pass_len,
			  0, 0, NULL, opt_timeout, 0, 0, cd);
	if (r < 0)
		goto out;
	...
	r = tpm_nv_define(cd, opt_tpmnv, tpm_pin, tpm_pin_len, opt_tpmpcr,
	...
	r = tpm_nv_write(cd, opt_tpmnv, tpm_pin, tpm_pin_len,
...
int main(int argc, const char **argv)
{
	...
	r = crypt_token_load("tpm2");
	if (r < 0) {
		l_err(NULL, "Failed to load tpm2 token handler.");
		return EXIT_FAILURE;
	}
